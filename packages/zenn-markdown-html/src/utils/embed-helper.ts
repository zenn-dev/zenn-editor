...
type EmbedGenerator = (token: string, options?: MarkdownOptions) => string;

/** 渡された文字列から埋め込み要素のHTMLを生成する関数をまとめたオブジェクト */
const embedGenerators: Record<EmbedType, EmbedGenerator> = {
  youtube(str) {
  ...
  tweet(str, options) {
    ...
    return generateEmbedServerIframe('tweet', str, options);
  },
  blueprintue(str) {
  ...
  card(str, options) {
    ...
    return generateEmbedServerIframe('card', str, options);
  },
  gist(str, options) {
     ...
     * のような形式
     */
    return generateEmbedServerIframe('gist', str, options);
  },
  github(str, options) {
    if (!isGithubUrl(str)) {
      return 'GitHub のファイルURLまたはパーマリンクを指定してください';
    }
    return generateEmbedServerIframe('github', str, options);
  },
  mermaid(src, options) {
    return generateEmbedServerIframe('mermaid', src, options);
  },
} as const;

const embedKeys = Object.keys(embedGenerators) as EmbedType[];
...
export function generateEmbedServerIframe(
  ...
  const origin = (() => {
    try {
      return new URL(options?.embedOrigin || '').origin;
    } catch {
      return void 0;
    }
  })();
...
export const generateEmbedHTML = (
  ...
  return isValid ? embedGenerators[type](str, options) : message;
};
...
export const generateLinkifyEmbedHTML = (
  ...
  if (isTweetUrl(url))
    return isValid ? embedGenerators.tweet(url, options) : msg;

  if (isYoutubeUrl(url))
    return isValid ? embedGenerators.youtube(url, options) : msg;

  // GitHub は URL が長くなりやすいためバリデーション(`validateEmbedToken`)から外す
  if (isGithubUrl(url))
    return generateEmbedServerIframe('github', url, options);

  return generateEmbedServerIframe('card', url, options);
};
