...
const renderLink = async (src: string) => {
  return markdownToHtml(src, options);
};

describe('Linkシンタックスのテスト', () => {
  test('リンクに変換する', async () => {
    const html = await renderLink('[example](https://example.com)');
    expect(html).toContain(
      '<a href="https://example.com" target="_blank" rel="nofollow noopener noreferrer">example</a>'
    );
  });
  test('hrefが"/"から始まる場合は属性のないリンクに変換する', async () => {
    const html = await renderLink('[Articles](/articles)');
    expect(html).toContain('<a href="/articles">Articles</a>');
  });
  test('hrefが"#"で始まる場合は属性のないリンクに変換する', async () => {
    const html = await renderLink('[Example](#example)');
...
describe('Linkifyのテスト', () => {
  describe('直書きのリンクのテスト', () => {
    ...
    test('リスト内のリンクをリンクカードに変換しない', async () => {
      const html = await renderLink('- https://example.com\n- second');
      const iframe = parse(html).querySelector('aside iframe');
      expect(iframe).toBeNull();
      expect(html).toContain(
        '<a href="https://example.com" target="_blank" rel="nofollow noopener noreferrer">https://example.com</a>'
    ...
    test('URLにテキストが続く場合はリンクカードに変換しない', async () => {
      const html = await renderLink('https://example.com foo');
      expect(html).toContain(
        '<a href="https://example.com" target="_blank" rel="nofollow noopener noreferrer">https://example.com</a> foo'
      );
    });

    test('同じ段落内のテキストを含むリンクをリンクカードに変換しない', async () => {
      const html = await renderLink(
        `a: https://example.com\nb: https://example.com`
      );
      expect(html).toContain(
    ...
    test('URLにテキストが続くならリンクが先頭であってもリンクカードに変換しない', async () => {
    ...
    test('URLの前にテキストがあるならリンクが行末でもリンクカードに変換しない', async () => {
      const html = await renderLink('text https://example.com\n\n');
      expect(html).toContain(
  ...
  describe('埋め込みiframeのテスト', () => {
    describe('LinkCardのテスト', () => {
      ...
      test('前の要素が<br />の場合はリンクカードに変換する', async () => {
        const url = 'https://example.com';
        const html = await renderLink(`foo\n${url}`);
        validateConvertLinkCardEmbeddedIframe(html, url);
      });

      test('改行で囲まれている場合はリンクカードに変換する', async () => {
        const url = 'https://example.com';
        const html = await renderLink(`\n\n${url}\n\n`);
        validateConvertLinkCardEmbeddedIframe(html, url);
      });

      test('前に改行があってリンクが段落の終わりならリンクカードに変換する', async () => {
        const url = 'https://example.com';
        const html = await renderLink(`text\n${url}\n\n`);
        validateConvertLinkCardEmbeddedIframe(html, url);
      });

      test('リンクが段落の先頭でその後に開業が続く場合はリンクカードに変換する', async () => {
        const url = 'https://example.com';
        const html = await renderLink(`\n\n${url}\ntext`);
        validateConvertLinkCardEmbeddedIframe(html, url);
      });

      test('リンクの前後に改行があるならリンクカードに変換する', async () => {
        const url = 'https://example.com';
        const html = await renderLink(`text\n${url}\ntext`);
        validateConvertLinkCardEmbeddedIframe(html, url);
      });

      test('<p />の最初の要素ならリンクカードに変換する', async () => {
        const url = 'https://example.com';
        const html = await renderLink(`foo\n\n${url}`);
      ...
      test('行内にリンクが存在する場合でも、各リンクが改行で区切られている場合は、リンクを変換する', async () => {
        const linkCardUrls = ['https://example1.com', 'https://example2.com'];
        const html = await renderLink(
          `${linkCardUrls[0]}\n${linkCardUrls[1]}\n`
        );
    ...
    describe('Tweet埋め込みのテスト', () => {
      describe('twitter.com', () => {
        test('ツイートリンクを<iframe />に変換する', async () => {
          const url = 'https://twitter.com/jack/status/20';
          const html = await renderLink(`${url}`);
          const iframe = parse(html).querySelector('span.zenn-embedded iframe');
        ...
        test('クエリ文字列を含むツイートリンクを埋め込み<iframe />に変換する', async () => {
    ...
    describe('<details /> 内のテスト', () => {
      ...
      test('<details />がネストされていても埋め込みiframeに変換する', async () => {
        const html = await renderLink(
          [
            `::::details example`,
