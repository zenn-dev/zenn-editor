...
describe('Docswell', () => {
  describe('Docswellの埋め込み用URLの場合', () => {
    test('Docswellのiframeを返すこと', async () => {
    ...
    test('ページ番号(ハッシュ形式)が指定されたDocswellのiframeを返すこと', async () => {
      const html = await markdownToHtml(
    ...
    test('ページ番号にXSS文字列が含まれる場合はエラーメッセージを返すこと', async () => {
      const html = await markdownToHtml(
  ...
  describe('DocswellのスライドURL（ページ番号付き）の場合', () => {
    test('ページ番号(パス形式)が指定されたDocswellのiframeを返すこと', async () => {
      const html = await markdownToHtml(
