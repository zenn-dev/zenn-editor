...
脚注の中から別の脚注を参照しているケース[^nested]です。ツールチップ内に表示される参照 `[n]` にホバーしてもポップアップは連鎖しません（クリックすると従来どおり脚注セクションへジャンプします）。

[^nested]: この脚注は別の脚注[^inner]を参照しています。さらにインライン記法^[入れ子になったインライン脚注です]も入れ子にできます。
[^inner]: 入れ子の参照先の脚注です。
